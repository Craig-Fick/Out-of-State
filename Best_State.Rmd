...
```
<br>
*___This data breaks race down into 6 categories along with respective percentages. Here is a visual for each state.___*
...
|   There are two tools to explore below. One is the interactive Tableau dashboard shown above, that I think works best as it allows you to adjust all the variables at once. The second tool is an application made in a package called shiny. This tool is good for visually exploring each variable one at a time.


## __Tableau Dashboard__

Click ![[HERE](https://public.tableau.com/shared/ZSMCTNWK9?:display_count=n&:origin=viz_share_link)] to use the Tableau Dashboard located in Tableau Public!


*__Note: You may need to create an account with Tableau Public to use. If the map goes completely black during use, this means there are no states fitting the range selected. You will need to readjust the ranges for the map to reappear.__*


## __Interactive Shiny APP__
...
```
*__Note: For the polotic variable 0 = Republican and 1 = Democrat.__*
